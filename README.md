...
**动态规划**适合用来求解最优问题，比如求最大值、最小值等，通常具备3个特征：
- 最优子结构：当前阶段后面阶段的状态可以通过前面阶段的状态推导出来
- 无后效性：在推导后面阶段的状态时，只用关心前面阶段的状态值
- 重复子问题：不同的决策序列在到达某个相同阶段时可能会产生的重复状态

解决动态规划问题，常用的有两种方法：
- 状态转移表法：回溯算法实现 -> 定义状态 -> 画递归树 -> 找重复子问题 -> 画状态转移方程 -> 根据递推关系填表 -> 将填表过程翻译成代码
- 状态转移方程法：找最优子结构 -> 写状态转移方程 -> 将状态转移方程翻译成代码

#### [爬楼梯](src/climbing_stairs)
#### [最小路径和](src/minimum_path_sum)
#### [三角形最小路径和](src/triangle)
#### [零钱兑换](src/coin_change)
#### [最长上升子序列](src/longest_increasing_subsequence)
#### [编辑距离](src/edit_distance)